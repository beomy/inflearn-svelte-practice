<script>
	...
	import { getSudoku, createSudokuV2, updateSudoku, getSudokuSolveV2, remainHint, message, memoFlag } from '@/store/sudoku';
	...
	async function getNewSudoku (difficulty) {
		await createSudokuV2({ difficulty });
		const result = await getSudoku();
		return result;
	}
	...
	async function handleSolveTest () {
		const { board, memo } = await getSudokuSolveV2({ board: sudoku.board });
		sudoku.answer = board;
		sudoku.memo = memo;
	}
</script>
...
<button on:click={handleSolveTest}>알고리즘 적용</button>
<DefaultLayout>
